...
models = ["CGal_4_20_0.01_E_nau","CGal_4_30_0.01_E_nau", "CGal_4_40_0.01_E_nau", "CGal_4_50_0.01_E_nau", "CGal_4_60_0.01_E_nau", "CGal_4_70_0.01_E_nau", "CGal_4_80_0.01_E_nau", "CGal_4_85_0.01_E_nau"] 
...
thicks = [30,30,30,30,30,30,30,30] """ 
...
len(masks) == len(models) == len(fitsnames) == len(beamsizes) == len(centres) == len(halfbeams) == len(thicks), "All parameter lists must have the same length"
...
for i in range(len(models)):
    ...
    print(f"Running BBB_template with  mask: {mask}, model: {model}, beamsize: {beamsize}, fitsname: {fitsname}, centre: {centre}, halfbeam: {halfbeam}, thick: {thick}") #vrot: {vrot}, vdisp: {vdisp}, inc: {inc}, phi: {phi}, dens: {dens}, fitting: {fitting},
    subprocess.run(['python', '/home/user/THESIS/scripts/BBB_test_subprocess.py', '--mask', mask, '--model', model,  '--beamsize', str(beamsize),  '--fitsname', fitsname, '--centre', str(centre),  '--halfbeam', str(halfbeam) , '--thick', str(thick)])# '--fitting', ','.join(fitting),
